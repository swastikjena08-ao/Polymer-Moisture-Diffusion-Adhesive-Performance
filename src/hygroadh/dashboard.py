...
class Dashboard:
    ...
    def close(self) -> None:
        ...
        _BOUND_PORTS.discard(self.port)
...
def build_dashboard(host: str = "127.0.0.1", port: int = 8765,
    ...
    for offset in range(attempts):
        ...
        if candidate in _BOUND_PORTS:
            last_error = OSError(f"port {candidate} is already in use on {host}")
            continue
        try:
            server = ThreadingHTTPServer((host, candidate), handler)
        except OSError as exc:
            # Linux/macOS use 98/48 for an in-use port; Windows raises 10048.
            if exc.errno not in (98, 48, 10048):
                raise
            last_error = exc
            continue
        server.daemon_threads = True
        dashboard = Dashboard(server=server, host=host, port=server.server_address[1])
        _BOUND_PORTS.add(dashboard.port)
        return dashboard
    raise HygroadhError(
