...
headline response is ``time_to_threshold_s``: the time for moisture at the
far face of the film to reach the configured threshold.

Unreachable thresholds are reported as ``inf`` rather than raised. A thick,
cold film may not cross the threshold inside the simulated window; in that case
the result is reported as surviving the full exposure.
"""
...
class SweepResult:
    ...
    def finite_fraction(self, name: str) -> float:
        ...
        Worth reporting alongside a ``time_to_threshold_s`` surface: if half the
        grid never reaches the threshold, a contour plot of it is mostly empty
        and the reader should be told so rather than left to infer it.
        """
