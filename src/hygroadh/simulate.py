"""Run a case and combine transport with the reported threshold criterion.

This module resolves the material and exposure settings before passing plain
values to the diffusion solvers:
...
class ExposureCondition:
    ...
    ----------
    temperature_k:
        Service temperature, K. Drives ``D`` and, if the isotherm has a
        sorption enthalpy, ``M_sat``.
    relative_humidity:
...
class Case:
    ...
    criterion: MoistureCriterion = field(default_factory=MoistureCriterion)
    ...
    def resolved_solver(self) -> str:
        ...
        if self.solver == "analytical" and self.requires_finite_volume:
            raise PhysicsError(
                "the analytical solution assumes a constant surface condition and "
                "no surface resistance; this case uses a humidity schedule or a "
                "surface transfer coefficient. Use solver='fd' or 'auto'."
            )
        return self.solver
...
class SimulationResult:
    ...
    def interface_pct(self) -> np.ndarray:
        """Absolute moisture content at the far face, wt%."""
        return self.transport.interface_pct
    ...
    def time_to_threshold(self) -> float:
        """Time for the far face to reach the moisture threshold, or ``inf``.

        The headline response: it collapses a whole simulation into one number
        that can be mapped over a design space.
        """
        return self.case.criterion.time_to_threshold(
            self.transport.time, self.interface_normalized, self.saturation_pct
        )

    @property
    def threshold_reachable(self) -> bool:
        """Whether the threshold is attainable at all at this saturation."""
        return self.case.criterion.is_reachable(self.saturation_pct)

    @property
    def threshold_pct(self) -> float:
        """The threshold expressed as an absolute water content, wt%."""
        return self.case.criterion.moisture_pct(self.saturation_pct)
    ...
    def summary(self) -> dict[str, object]:
        ...
        return {
            ...
            "final_uptake_pct": float(self.transport.uptake_pct[-1]),
            "final_interface_pct": float(self.interface_pct[-1]),
            "time_to_half_uptake_s": self.time_to_half_uptake,
            "threshold_value": self.case.criterion.value,
            "threshold_basis": self.case.criterion.basis,
            "threshold_pct": self.threshold_pct,
            "threshold_reachable": self.threshold_reachable,
            "time_to_threshold_s": self.time_to_threshold,
        }
