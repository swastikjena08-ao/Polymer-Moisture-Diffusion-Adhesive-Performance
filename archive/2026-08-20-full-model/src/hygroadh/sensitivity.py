...
RESPONSES: dict[str, Callable[[object], float]] = {
    "final_ari": lambda result: float(result.final_index),
    "time_to_ari_threshold": lambda result: float(result.time_to_ari_threshold),
...
def _require_usable(value: float, response: str, context: str) -> float:
    if not np.isfinite(value):
        raise PhysicsError(
            ...
            "'time_to_ari_threshold' this means the threshold is never reached "
            "in the simulated window: lengthen the exposure, raise the "
            "threshold, or use 'final_ari', which is always finite."
        )
    if value <= 0.0:
        raise PhysicsError(
...
def local_elasticities(case: Case, response: str = "time_to_ari_threshold",
    ...
    ``time_to_ari_threshold`` is the default rather than ``final_ari`` because
    of a degeneracy worth knowing about: once an exposure is long enough to
    saturate the film, every case ends at ``M/M_inf = 1`` and the *final* ARI
    becomes exactly independent of thickness and diffusivity --- those two
    factors set how fast the bondline wets, not where it ends up. Their
    elasticities then come back as precisely zero, which is correct but useless
    for ranking. The time to cross a threshold stays sensitive to all three.
    """
...
def default_ranges(case: Case, factor_span: float = 3.0,
    ...
    Thickness and diffusivity get a geometric span; temperature gets an
    additive one. The upper temperature is capped well below the dry glass
    transition, because above it there is no dry adhesion baseline for ARI to be
    a fraction of and the model correctly refuses to evaluate.
    """
    ...
    ceiling = case.polymer.tg_dry_k - 25.0
    if temperature >= ceiling:
        raise PhysicsError(
            f"base temperature {temperature:.1f} K is within 25 K of the dry glass "
            f"transition {case.polymer.tg_dry_k:.1f} K, leaving no headroom to "
            "screen temperature; lower it or raise tg_dry_k"
        )
    return {
        "thickness": (thickness / span, thickness * span),
        "temperature_k": (
            max(temperature - temperature_span, 200.0),
            min(temperature + temperature_span, ceiling),
        ),
...
def morris_screening(case: Case, ranges: dict[str, tuple[float, float]] | None = None,
                     response: str = "time_to_ari_threshold", n_trajectories: int = 10,
