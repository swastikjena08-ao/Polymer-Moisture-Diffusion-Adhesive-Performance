...
Handles everything the analytical solution cannot: concentration-dependent
diffusivity, a surface condition that varies in time, and finite surface
...
def _face_conductance(u: np.ndarray, d_ref: float, beta: float, dz: float,
                      surface_transfer: float | None) -> tuple[float, np.ndarray]:
    """Return the surface conductance and the interior face conductances.

    Conductances have units of m/s, so that ``g * delta_u`` is a flux.
    Interior faces use the diffusivity at the arithmetic mean of the two
    neighbouring cell values.
    """
    if beta == 0.0:
        d_interior = np.full(u.size - 1, d_ref)
        d_surface = d_ref
    else:
        d_interior = d_ref * np.exp(beta * 0.5 * (u[:-1] + u[1:]))
        d_surface = d_ref * float(np.exp(beta * u[0]))
    g_interior = d_interior / dz
    half_cell_resistance = dz / (2.0 * d_surface)
...
def _operator(u: np.ndarray, d_ref: float, beta: float, dz: float,
    ...
    n = u.size
    g_s, g = _face_conductance(u, d_ref, beta, dz, surface_transfer)
...
def _step(u_old: np.ndarray, dt: float, theta: float, d_ref: float, beta: float,
          dz: float, surface_transfer: float | None, surface_old: float,
          surface_new: float, picard_tol: float, picard_max_iter: int) -> np.ndarray:
    """Advance one time step of the theta-method, with Picard iteration on D(u)."""
    lo_o, di_o, up_o, src_o = _operator(
        u_old, d_ref, beta, dz, surface_transfer, surface_old
    )
    explicit = (1.0 - theta) * dt
    rhs_base = u_old + explicit * (
        _apply_bands(lo_o, di_o, up_o, u_old) + src_o
    )

    guess = u_old
    iterations = 1 if beta == 0.0 else picard_max_iter
    for _ in range(iterations):
        lo, di, up, src = _operator(
            guess, d_ref, beta, dz, surface_transfer, surface_new
        )
        rhs = rhs_base + theta * dt * src
        candidate = solve_tridiagonal(
            -theta * dt * lo, 1.0 - theta * dt * di, -theta * dt * up, rhs
        )
        change = float(np.max(np.abs(candidate - guess)))
        guess = candidate
    ...
    raise ConvergenceError(
        f"Picard iteration for concentration-dependent diffusivity did not "
        f"converge in {picard_max_iter} iterations (last change {change:.3e}); "
        "reduce conc_dependence or the time step"
    )
...
def solve(
    ...
    diffusivity:
        Diffusivity in m^2/s at zero moisture content, already corrected to
        ``temperature_k``.
    saturation_pct:
    ...
    conc_dependence:
        ``beta_c`` in ``D(u) = D_ref exp(beta_c u)``. Non-zero triggers
        Picard iteration each step.
    surface_transfer:
    ...
    surface_fn, surface_max = _as_callable(surface)
    d_max = d_ref * float(np.exp(conc_dependence * max(surface_max, initial, 1.0)))
    dt_diffusive = cfl * dz**2 / d_max
