...
def _print_summary(summary: dict) -> None:
    order = (
        ...
        ("final_interface_normalized", "final bondline C/Csat"),
        ("final_ari", "final ARI"),
        ("dominant_mechanism", "binding mechanism"),
    )
    ...
    threshold = float(summary["ari_threshold"])
    for label, seconds in (
        ("time to half uptake", summary["time_to_half_uptake_s"]),
        (f"time to ARI {threshold:.2f}", summary["time_to_ari_threshold_s"]),
    ):
        print(f"  {label:<{width}} : {_format_time(seconds)}")


STANDING_NOTE = (
    "Note: the transport half is verified against exact analytical solutions.\n"
    "The Adhesion Retention Index is a phenomenological model with parameters\n"
    "you supply; treat it as a comparative index for ranking designs, not as a\n"
...
def cmd_sweep(args: argparse.Namespace) -> int:
    ...
    finite = result.finite_fraction("time_to_ari_threshold_s")
    print(f"time to ARI threshold reached at {finite:.0%} of grid points")
    if finite < 1.0:
        print("  (the rest survive the whole exposure; reported as 'not reached')")

    surface = result.surface("time_to_ari_threshold_s")
    reachable = surface[np.isfinite(surface)]
    if reachable.size:
        print(f"  fastest failure: {_format_time(float(reachable.min()))}")
        print(f"  slowest failure: {_format_time(float(reachable.max()))}")
...
def cmd_info(args: argparse.Namespace) -> int:
    ...
    print(f"  dry Tg                : {to_celsius(polymer.tg_dry_k):.3g} degC")
    saturated = polymer.saturation_pct(temperature, condition.relative_humidity)
    print(f"  wet Tg at saturation  : "
          f"{to_celsius(polymer.glass_transition_k(saturated)):.3g} degC")
    print(f"  ARI threshold         : {case.ari_threshold:.3g}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="hygroadh",
        description=(
            "Diffusion-based prediction of moisture uptake and adhesion "
            "retention in polymer films."
        ),
    )
    ...
    sens_parser.add_argument("--response", default="time_to_ari_threshold",
