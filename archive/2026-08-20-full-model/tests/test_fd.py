...
def test_concentration_dependent_diffusivity_accelerates_uptake():
    t = _times()
    fickian = fd.solve(t, THICKNESS, DIFFUSIVITY, 2.0, TEMPERATURE, n_cells=40)
    plasticizing = fd.solve(t, THICKNESS, DIFFUSIVITY, 2.0, TEMPERATURE, n_cells=40,
                            conc_dependence=1.5)
    assert np.all(plasticizing.uptake_normalized >= fickian.uptake_normalized - 1e-12)
    mid = len(t) // 2
    assert plasticizing.uptake_normalized[mid] > 1.05 * fickian.uptake_normalized[mid]
    # Both must still saturate at the same equilibrium.
    assert plasticizing.uptake_normalized[-1] == pytest.approx(1.0, abs=5e-3)
