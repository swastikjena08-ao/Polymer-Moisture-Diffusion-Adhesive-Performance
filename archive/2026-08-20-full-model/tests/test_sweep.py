...
#: See test_sensitivity: the exact power laws hold when wetting sets the life.
...
def test_grid_shape_and_axis_order_are_canonical():
    ...
    assert result.surface("final_ari").shape == (3, 2)
...
def test_time_to_threshold_scales_with_thickness_squared():
    ...
    Stated for the wetting-limited regime. Under the package defaults slow
    hydrolysis sets the life and thickness buys much less --- see
    test_physics_trends.
    """
    thickness = np.array([1e-4, 2e-4, 4e-4])
    result = sweep.run_sweep(
        _case(adhesion=WETTING_LIMITED, ari_threshold=0.8), {"thickness": thickness}
    )
    times = result.surface("time_to_ari_threshold_s")
...
def test_time_to_threshold_is_inversely_proportional_to_diffusivity():
    ...
    result = sweep.run_sweep(
        _case(adhesion=WETTING_LIMITED, ari_threshold=0.8), {"diffusivity": diffusivity}
    )
    times = result.surface("time_to_ari_threshold_s")
    ratios = times[:-1] / times[1:]
...
def test_unreachable_thresholds_are_infinite_and_counted_not_raised():
    ...
    assert result.finite_fraction("time_to_ari_threshold_s") == 0.0
    # The always-finite responses must still be usable.
    assert np.all(np.isfinite(result.surface("final_ari")))
...
def test_records_carry_the_swept_axes_alongside_the_summary():
    ...
    assert "final_ari" in row and "dominant_mechanism" in row
