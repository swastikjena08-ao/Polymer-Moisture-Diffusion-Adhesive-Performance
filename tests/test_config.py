...
def test_the_worked_example_defines_a_sweep():
    ...
    assert axes.temperature_k is not None and axes.temperature_k.size == 7
    assert axes.thickness[0] == pytest.approx(50e-6)


def test_the_threshold_section_sets_the_criterion():
    data = {**MINIMAL, "threshold": {"basis": "wt_pct", "value": 1.2}}
    criterion = build_configuration(data).case.criterion
    assert criterion.basis == "wt_pct"
    assert criterion.value == pytest.approx(1.2)


def test_the_threshold_defaults_differ_sensibly_by_basis():
    """A bare fraction and a bare weight percent need different defaults."""
    fraction = build_configuration({**MINIMAL, "threshold": {}}).case.criterion
    assert fraction.basis == "normalized" and fraction.value == pytest.approx(0.5)
    absolute = build_configuration(
        {**MINIMAL, "threshold": {"basis": "wt_pct"}}).case.criterion
    assert absolute.value == pytest.approx(1.0)


def test_an_unknown_threshold_basis_is_reported():
    with pytest.raises(ConfigError, match="basis must be one of"):
        build_configuration({**MINIMAL, "threshold": {"basis": "percent"}})


def test_activation_energy_accepts_kilojoules_or_joules():
    """kJ/mol is how every source quotes it, so demanding J/mol invites a 1000x slip."""
    kilo = {**MINIMAL, "polymer": {"diffusivity_ref": 5e-13, "activation_energy_kj": 50,
                                   "isotherm": {"m_ref_pct": 2.0}}}
    joule = {**MINIMAL, "polymer": {"diffusivity_ref": 5e-13, "activation_energy": 50e3,
                                    "isotherm": {"m_ref_pct": 2.0}}}
    assert build_configuration(kilo).case.polymer.activation_energy == pytest.approx(50e3)
    assert build_configuration(joule).case.polymer.activation_energy == pytest.approx(50e3)
    both = {**MINIMAL, "polymer": {"diffusivity_ref": 5e-13, "activation_energy": 50e3,
                                   "activation_energy_kj": 50,
                                   "isotherm": {"m_ref_pct": 2.0}}}
    with pytest.raises(ConfigError, match="exactly one"):
        build_configuration(both)


def test_the_isotherm_takes_an_optional_sorption_enthalpy():
    data = {**MINIMAL, "polymer": {"diffusivity_ref": 5e-13,
                                   "isotherm": {"m_ref_pct": 2.0,
                                                "enthalpy_sorption": 6e3}}}
    assert build_configuration(data).case.polymer.isotherm.enthalpy_sorption == 6e3


def test_the_removed_advanced_keys_are_now_rejected():
    """The simplified model has no Tg, no D(C), and no isotherm exponent.

    Rejecting them is better than ignoring them: a config carried over from the
    fuller model fails loudly instead of running with the key silently dropped.
    """
    for section, key, value in (
        ("polymer", "tg_dry_c", 120),
        ("polymer", "conc_dependence", 1.0),
        ("threshold", "hydrolysis_rate_ref", 1e-7),
        ("threshold", "knockdown_per_pct", 0.5),
    ):
        data = {**MINIMAL, section: {**MINIMAL.get(section, {}), key: value}}
        if section == "polymer":
            data["polymer"] = {"diffusivity_ref": 5e-13,
                               "isotherm": {"m_ref_pct": 2.0}, key: value}
...
def test_missing_required_keys_are_reported_with_their_path():
    data = {**MINIMAL, "polymer": {"isotherm": {"m_ref_pct": 2.0}}}
...
def test_physically_invalid_values_surface_as_config_errors_naming_the_section():
    ...
    with pytest.raises(ConfigError, match=r"polymer\.isotherm: .*m_ref_pct"):
        build_configuration({**MINIMAL, "polymer": {
            "diffusivity_ref": 5e-13, "isotherm": {"m_ref_pct": -1.0}}})
    with pytest.raises(ConfigError, match=r"threshold: "):
        build_configuration({**MINIMAL, "threshold": {"value": -1.0}})
