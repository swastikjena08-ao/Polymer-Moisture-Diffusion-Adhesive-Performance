...
def test_history_csv_has_a_row_per_sample_and_the_declared_columns(tmp_path, result):
    ...
    assert float(rows[-1]["far_face_pct"]) == pytest.approx(
        result.interface_pct[-1], rel=1e-6
    )
...
def test_summary_json_replaces_infinities_with_a_readable_marker(tmp_path):
    ...
    assert payload["time_to_threshold_s"] == "not reached"
...
def test_summary_json_of_a_real_run_is_strictly_parseable(tmp_path, result):
    ...
    payload = json.loads(path.read_text())
    assert 0.0 <= payload["final_interface_normalized"] <= 1.0
    assert payload["final_interface_pct"] >= 0.0
    assert payload["threshold_basis"] in ("normalized", "wt_pct")
