...
def test_requesting_the_analytical_solver_for_an_unsupported_case_is_refused():
    ...
    with pytest.raises(PhysicsError, match="constant surface condition"):
...
def test_the_two_solvers_agree_on_a_case_both_can_handle():
    ...
    assert numeric.interface_normalized == pytest.approx(
        exact.interface_normalized, abs=2e-3
    )
    assert numeric.time_to_threshold == pytest.approx(
        exact.time_to_threshold, rel=5e-3
    )
...
def test_the_schedule_is_mapped_through_the_isotherm():
    """Henry's law is linear, so half the humidity is half the surface value.

    The mapping still goes through the isotherm rather than assuming
    proportionality, which is what lets a curved isotherm be swapped in later
    ...
    surface = surface_condition(polymer, condition, saturation)
    assert surface(0.0) == pytest.approx(0.5)


def test_a_drying_leg_lets_uptake_and_far_face_moisture_fall_back():
    polymer = _polymer()
    wet = from_days(20)
    condition = ExposureCondition(
    ...
    # Far-face moisture follows the exposure back down; nothing accumulates.
    assert result.interface_normalized.max() > 0.5
    assert result.interface_normalized[-1] < 0.2
...
def test_an_unreachable_threshold_is_reported_as_infinite_not_raised():
    """A cold thick film legitimately never crosses the threshold."""
    # Equilibrium uptake here is 0.96 wt%, so a 2 wt% threshold at the far face
    # is unreachable by construction, not merely slow.
    case = _case(
        thickness=2e-3,
    ...
    assert run(case).time_to_threshold == np.inf
...
def test_summary_reports_every_scalar_a_report_needs():
    ...
    for key in (
        ...
        "time_to_half_uptake_s", "time_to_threshold_s",
        "threshold_pct", "threshold_reachable", "final_interface_pct", "solver",
    ):
...
def test_rejects_invalid_cases_and_conditions():
    ...
    with pytest.raises(PhysicsError):
        _case(solver="magic")
    with pytest.raises(PhysicsError):
        _case(criterion=MoistureCriterion(1.5, "normalized"))
